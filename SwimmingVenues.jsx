...
import { Waves, Frown } from "lucide-react";
import VenueCard from "../components/venues/VenueCard";
import VenueFilters from "../components/venues/VenueFilters";

const SWIMMING_CONFIG = {
  name: "Swimming",
  icon: Waves,
  venueTypes: {
    public: "Public Pools",
    ...
    gym: "Gym Pools",
    recreation_center: "Rec Centers"
  }
};

export default function SwimmingVenues() {
    ...
    for (const venue of venuesWithoutImages) {
      ...
      try {
        const result = await InvokeLLM({
          prompt: `Find a high-quality, publicly usable image URL for the swimming facility "${venue.name}" in "${venue.location}". Only return the direct image URL.`,
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      const query = { sport_type: "swimming" };
      const data = await Venue.filter(query, "-rating");
      setVenues(data);
      fetchImagesForVenues(data);
    } catch (error) {
      console.error("Error loading initial data:", error);
      const data = await Venue.filter({ sport_type: "swimming" }, "-rating");
      setVenues(data);
      fetchImagesForVenues(data);
    } finally {
      setIsLoading(false);
        ...
        >
          <div className="flex items-center space-x-3 mb-6">
            <div className="w-12 h-12 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-xl flex items-center justify-center">
              <SWIMMING_CONFIG.icon className="w-6 h-6 text-white" />
            </div>
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Swimming Pools</h1>
