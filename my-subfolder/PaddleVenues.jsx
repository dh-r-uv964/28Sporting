...
const PADDLE_CONFIG = {
  ...
  venueTypes: {
    ...
    simulator: "Indoor Facilities",
  }
};

export default function PaddleVenues() {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      const query = location ? { sport_type: "paddle", location } : { sport_type: "paddle" };
      const data = await Venue.filter(query, "-rating");
      setVenues(data);
      fetchImagesForVenues(data);
    } catch (error) {
      console.error("Error loading initial data:", error);
      const data = await Venue.filter({ sport_type: "paddle" }, "-rating");
      setVenues(data);
      fetchImagesForVenues(data);
    } finally {
      setIsLoading(false);
    ...
    for (const venue of venuesWithoutImages) {
      ...
      try {
        const result = await InvokeLLM({
          prompt: `Find a high-quality, scenic, publicly usable image URL for the paddle tennis court "${venue.name}" in "${venue.location}". Only return the direct image URL.`,
        ...
        >
          <div className="flex items-center space-x-3 mb-6">
            <div className="w-12 h-12 bg-gradient-to-r from-purple-500 to-pink-600 rounded-xl flex items-center justify-center">
              <PADDLE_CONFIG.icon className="w-6 h-6 text-white" />
            </div>
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Paddle Courts</h1>
