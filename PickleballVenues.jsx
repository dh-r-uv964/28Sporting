...
const PICKLEBALL_CONFIG = {
  ...
  venueTypes: {
    ...
    simulator: "Gyms & Rec Centers",
  }
};

export default function PickleballVenues() {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      const query = location ? { sport_type: "pickleball", location } : { sport_type: "pickleball" };
      const data = await Venue.filter(query, "-rating");
      setVenues(data);
      fetchImagesForVenues(data);
    } catch (error) {
      console.error("Error loading initial data:", error);
      const data = await Venue.filter({ sport_type: "pickleball" }, "-rating");
      setVenues(data);
      fetchImagesForVenues(data);
    } finally {
      setIsLoading(false);
    ...
    for (const venue of venuesWithoutImages) {
      ...
      try {
        const result = await InvokeLLM({
          prompt: `Find a high-quality, scenic, publicly usable image URL for the pickleball court "${venue.name}" in "${venue.location}". Only return the direct image URL.`,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="flex items-center space-x-3 mb-6">
            <div className="w-12 h-12 bg-gradient-to-r from-orange-500 to-red-600 rounded-xl flex items-center justify-center">
              <PICKLEBALL_CONFIG.icon className="w-6 h-6 text-white" />
            </div>
            <div>
        ...
        {filteredVenues.length === 0 && !isLoading && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">No Pickleball Courts Found</h3>
