...
const TENNIS_CONFIG = {
  ...
  venueTypes: {
    ...
    simulator: "Indoor Centers",
  }
};

export default function TennisVenues() {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      const query = location ? { sport_type: "tennis", location } : { sport_type: "tennis" };
      const data = await Venue.filter(query, "-rating");
      setVenues(data);
      fetchImagesForVenues(data);
    } catch (error) {
      console.error("Error loading initial data:", error);
      const data = await Venue.filter({ sport_type: "tennis" }, "-rating");
      setVenues(data);
      fetchImagesForVenues(data);
    } finally {
      setIsLoading(false);
    ...
    for (const venue of venuesWithoutImages) {
      ...
      try {
        const result = await InvokeLLM({
          prompt: `Find a high-quality, scenic, publicly usable image URL for the tennis facility "${venue.name}" in "${venue.location}". Only return the direct image URL.`,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="flex items-center space-x-3 mb-6">
            <div className="w-12 h-12 bg-gradient-to-r from-blue-500 to-cyan-600 rounded-xl flex items-center justify-center">
              <TENNIS_CONFIG.icon className="w-6 h-6 text-white" />
            </div>
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Tennis Courts</h1>
        ...
        {filteredVenues.length === 0 && !isLoading && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">No Tennis Courts Found</h3>
