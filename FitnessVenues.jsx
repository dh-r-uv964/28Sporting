...
export default function FitnessVenues() {
    ...
    for (const venue of venuesWithoutImages) {
      ...
      try {
        const result = await InvokeLLM({
          prompt: `Find a high-quality, publicly usable image URL for the fitness gym or studio "${venue.name}" in "${venue.location}". Only return the direct image URL.`,
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      const query = { sport_type: "fitness" };
      const data = await Venue.filter(query, "-rating");
      setVenues(data);
      fetchImagesForVenues(data);
    } catch (error) {
      console.error("Error loading initial data:", error);
      const data = await Venue.filter({ sport_type: "fitness" }, "-rating");
      setVenues(data);
      fetchImagesForVenues(data);
    } finally {
      setIsLoading(false);
        ...
        >
          <div className="flex items-center space-x-3 mb-6">
            <div className="w-12 h-12 bg-gradient-to-r from-purple-600 to-indigo-700 rounded-xl flex items-center justify-center">
              <FITNESS_CONFIG.icon className="w-6 h-6 text-white" />
            </div>
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Fitness Centers & Gyms</h1>
